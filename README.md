...
import { env } from 'sugar-env'
  ...
  env.current(): Environment
    e.g.: `env.current()`
      ...
      ✔ returns `"dev"` when `process.env.NODE_ENV` isn't one of: `"dev"`, `"test"`, `"ci"`, `"staging"`, `"review"` or `"prod"`
...
import { env } from 'sugar-env'
  ...
  env.mustBe(assertionEnvironmentName: string): void
    e.g.: `env.mustBe(env.DEV)`
      ✔ throws `EnvironmentAssertionError` when the current environment is different than the one specified
      ✔ does nothing when the current environment is the same as the one specified
...
import { env } from 'sugar-env'
  env(envVarName: string | string[]): string | null
    ...
    e.g.: `env(['PROD_S3_KEY_SECRET', 'S3_KEY_SECRET'])`
      ...
      ✔ returns the value of the first found non-empty environment variable
...
import { env, is } from 'sugar-env'
  env<T>(environmentName: string | string[], plugs: T & ChainablePlugs<string | null, T>): PipelinedReturnType<string | null, typeof plugs>
    ...
    can use a custom plug:
      ...
      const myPlug = {
        name: 'myPlug',
        fn: (ctx: Context<string | null>): Context<string> => {
          return (
            ctx.value === null
              ? { ...ctx, value: 'foo' }
              : ctx
